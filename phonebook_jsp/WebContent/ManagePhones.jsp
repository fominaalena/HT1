...
<body>
	...
	<form action="<%=request.getContextPath()%>/" method="post">
		<input type="hidden" name="id" value="<%=person.getId()%>" /> <input
			type="hidden" name="id" value="<%=phone.getId()%>" />
		<table align="center" border="1" width="70%">
			...
			<tr>
				...
				<td><input type="text" name="number"
					value="<%=phone.getNumber()%>" /></td>
			</tr>

			<tr>
				<td colspan="2" align="center"><input type="hidden" name="id"
					value="<%=person.getId()%>" /> <input type="submit"
					name="<%=jsp_parameters.get("next_action")%>"
					value="<%=jsp_parameters.get("next_action_label")%>" /></td>
			</tr>
			<tr>
				<td colspan="2" align="center"><a
					href="<%=request.getContextPath()%>/?action=back_to_managePerson&id=<%=person.getId()%>">Вернуться
