...
<body>
    ...
    %>
			...
			<tr>
				...
				<td><table>
						<%
							for (Map.Entry entry : person.getPhones().entrySet()) {
						%>
						<tr>
							<td><%=entry.getValue()%></td>
							<td><a
								href="<%=request.getContextPath()%>/?action=edit_phone&id=<%=entry.getKey()%>">Редактировать</a></td>
							<td><a
								href="<%=request.getContextPath()%>/?action=delete_phone&id=<%=entry.getKey()%>">Удалить</a></td>
							<%
								}
							%>
						</tr>
						<tr>
							<td><a
								href="<%=request.getContextPath()%>/?action=add_phone&id=<%=person.getId()%>">Добавить</a></td>
						</tr>
					</table></td>
			</tr>
			<%
			...
			<tr>
				<td colspan="2" align="center"><a
					href="<%=request.getContextPath()%>/?action=back_to_list">Вернуться
						к списку</a></td>
			</tr>
		</table>
	</form>
